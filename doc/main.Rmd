...
if (sample.reweight){
  xgb_tm_train <- system.time(xgb_fit_train <- xgbtrain(feature_train, label_train, w = weight_train, eta_val = 0.08 ,gamma = 0.4, lmd = par_best, nr = 200, md = 5))
} else {
  xgb_tm_train <- system.time(xgb_fit_train <- xgbtrain(feature_train, label_train, w = NULL, eta_val = 0.08 ,gamma = 0.4, lmd = par_best, nr = 200, md = 5))
}
save(xgb_fit_train, file="../output/xgb_fit_train.RData")
```
...
if(run.test){
  ...
  xgb_tm_test <- system.time({prob_pred <- xgbtest(xgb_fit_train, feature_test);
...
cat("The accuracy of model:", model_labels[which.min(xgb_res_cv$mean_error)], "is", accu_xgboost*100, "%.\n")
cat("The AUC of model:", model_labels[which.min(xgb_res_cv$mean_error)], "is", auc_xgboost, ".\n")
