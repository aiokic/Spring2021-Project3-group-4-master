...
xgbcv.function <- function(features, labels, K, 
                           eta_val = 0.05, 
                           lmd = 0.005 , 
                           nr = 500, 
                           md = 5,
  ...
  for (i in 1:K){
    ...
    ## model training
    if (reweight){
      model_train <- xgbtrain(feature_train_cv, label_train_cv, w = weight_train_cv, 
                              eta_val = eta_val, 
                              lmd = lmd , 
                              nr = nr,
                              md = md,
      ...
      model_train <- xgbtrain(feature_train_cv, label_train_cv, w = NULL, 
                              eta_val = eta_val, 
                              lmd = lmd , 
                              nr = nr,
                              md = md,
    ...
    ## make predictions
    prob_pred <- xgbtest(model_train, feature_test_cv)
    label_pred <- ifelse(prob_pred >= 0.5, 1, 0)
    label_test_cv_t <- ifelse(label_test_cv == 2, 0, 1)
    cv.error[i] <- 1 - sum(weight_test_cv * (label_pred == label_test_cv_t)) / sum(weight_test_cv)
    tpr.fpr <- WeightedROC(prob_pred, label_test_cv_t, weight_test_cv)
...
#res_cv_temp <- matrix(0, nrow = 1, ncol = 4)
#res_cv_temp[i,] <- xgbcv.function(features = feature_train, labels = label_train, K,eta_val =0.05,
